...
if command -v nix &> /dev/null; then
    ...
    echo ""
    echo "方法1: Holonix Flakeを直接使用（推奨、sudo不要）"
    echo "  nix run --accept-flake-config \"github:holochain/holonix?ref=main-0.5#hc\" -- --version"
    echo ""
    echo "  DNAパッケージ生成時:"
    echo "    nix run --accept-flake-config \"github:holochain/holonix?ref=main-0.5#hc\" -- dna pack dna/"
    echo ""
    echo "方法2: Holochain公式セットアップスクリプト（sudo権限が必要）"
    echo "  bash <(curl https://holochain.github.io/holochain/setup.sh)"
    echo "  注意: このスクリプトはsudo権限が必要で、対話的な操作が必要です"
    echo ""
    echo "インストール確認:"
